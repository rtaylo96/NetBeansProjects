...
public class Mouse extends AbstractMouse {
    ...
    public void move(List<Location> emptyLocations) {
         ...
         */
        // Check if emptyLocations is empty
        if (emptyLocations.isEmpty()) {
            // try moving to the popped crumbs to retrace your steps
            // but catch the ECE
            try {
                moveTo(crumbs.pop());
            ...
            // if empyLocations is not empty
        } else {
            // add current location to the stack 
            crumbs.push(getLocation());
            // then move to the emptyLocation which is a random spot
            moveTo(emptyLocations.get((int) +(Math.random() * emptyLocations.size())));
        }
    }
